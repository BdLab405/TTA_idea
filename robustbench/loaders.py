...
class Custom1DDataset(data.Dataset):
    """
    专门处理 1D 信号的数据集类
    """
    def __init__(self, samples, transform=None):
        super(Custom1DDataset, self).__init__()
        self.samples = samples
        # 注意：对于 1D 信号，通常不使用 torchvision 的 transform (如 Resize/Crop)
        # 而是使用自定义的信号增强，或者在此处留空
        self.transform = transform
    def __getitem__(self, index):
        # sample 结构: [signal_array, label_int, domain_str]
        signal, label, domain = self.samples[index]

        # 1. 转为 Tensor
        if not isinstance(signal, torch.Tensor):
            signal = torch.tensor(signal, dtype=torch.float32)

        # 2. 增加 Channel 维度: (2048,) -> (1, 2048)
        # 这一步是为了适配 Conv1d 的输入要求 [Batch, Channel, Length]
        if signal.dim() == 1:
            signal = signal.unsqueeze(0)

        # 3. Z-score 标准化 (Instance Normalization)
        # 这一步至关重要！模型没有 BN 层能完全替代输入的标准化
        mean = signal.mean()
        std = signal.std() + 1e-8
        signal = (signal - mean) / std

        # 4. 如果有额外的 Transform (例如加噪、掩码等 TTA 操作)
        if self.transform is not None:
            signal = self.transform(signal)

        # ★ 确保 label 是 PyTorch Tensor 且是 long 类型
        if not isinstance(label, torch.Tensor):
            label = torch.tensor(label, dtype=torch.long)
        # 如果 label 已经是 tensor，但可能类型不对 (比如 float)，则需要强制转换
        elif label.dtype != torch.long:
            label = label.long()

        return signal, label, domain  # 直接返回处理好的 label
    def __len__(self):
        return len(self.samples)
