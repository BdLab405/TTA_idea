...
class MY(TTAMethod):
    ...
    def forward_and_adapt(self, x):
        ...
        denom = (max_weight - min_weight + self.eps)

        # 构建新的 param groups（与原实现一致 —— 会丢失 optimizer state）
        new_param_groups = []
        for k, p in self.trainable_dict.items():
            if isinstance(self.grad_weight[k], float):
                lr_weight = 1.0
            else:
                value = (self.grad_weight[k].mean().item()) ** 0.5
                lr_weight = (value - min_weight) / denom
            param_lr = self.base_lr * (max(lr_weight, 1e-12) ** self.tau)
        ...
        # ---------- inplace LAW-based learning rate adaptation (no optimizer rebuild) ----------
        # 计算梯度平方的均值
        grad_sq = []
        for name, p in self.model.named_parameters():
            if p.grad is None:
                continue
            g = p.grad.detach()
            grad_sq.append(g.pow(2).mean())
        if len(grad_sq) == 0:
            return

        grad_sq = torch.stack(grad_sq)

        # min-max 归一化
        min_val, max_val = grad_sq.min(), grad_sq.max()
        if (max_val - min_val) < 1e-12:
            grad_norm = torch.ones_like(grad_sq)
        else:
            grad_norm = (grad_sq - min_val) / (max_val - min_val)

        # 更新每个参数组的 lr （注意 param_group 数量与模型参数数量一致时才更新）
        idx = 0
        for group in self.optimizer.param_groups:
            lr_scale = grad_norm[idx].item() if idx < len(grad_norm) else 1.0
            base_lr = self.cfg.MY.LAW_LR_BASE if hasattr(self.cfg.MY, "LAW_LR_BASE") else 1e-3
            group["lr"] = base_lr * (0.5 + lr_scale)  # 缩放学习率
            idx += 1
        ...
        if getattr(self, 'mixed_precision', False) and self.device == "cuda":
            # 兼容 mixed precision（假设 self.scaler 已存在）
