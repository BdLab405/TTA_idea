...
# Log datetime
_C.LOG_TIME = -1
_C.LOG_SUFFIX = -1      # 新增字段，用于批处理区分日志文件
...
# 质心损失权重
_C.COTTA.LAMBDA_CENTROID = 0.1
_C.COTTA.WARMUP_BATCHES = 5
_C.COTTA.MEMORY_TP = 5
_C.COTTA.SP_PROB = 0.01
_C.COTTA.GAUSSIAN_STD = 0.01
# --------------------------------- GTTA options ---------------------------- #
_C.GTTA = CfgNode()
...
_C.RMT.LAMBDA_CE_SRC = 0          # Lambda for source replay. Set to 0 for source-free variant
...
_C.MY.LAMBDA_CE_SRC = 0          # Lambda for source replay. Set to 0 for source-free variant
_C.MY.LAMBDA_CE_TRG = 1.0          # Lambda for self-training
_C.MY.LAMBDA_CONT = 1.0            # Lambda for contrastive learning
_C.MY.NUM_SAMPLES_WARM_UP = 50000  # Number of samples used during the mean teacher warm-up

_C.MY.LAW_LR_BASE = 1e-3     # LAW部分的基础学习率
_C.MY.LAW_TAU = 2.0          # LAW的温度系数（可选，如果你融合时用了）
...
_C.SOURCE.PERCENTAGE = 1   # (0, 1] Possibility to reduce the number of source samples
...
def assert_and_infer_cfg():
    """断言检查配置是否有效。主要是提前验证某些关键字段是否属于支持范围。"""
    err_str = "Unknown adaptation method."
    # 检查当前模型适应方法是否合法（仅支持 source、norm、tent）
    assert _C.MODEL.ADAPTATION in ["source", "norm", "tent"]

    err_str = "Log destination '{}' not supported"
    # 检查日志输出方式是否合法（仅支持 stdout 和 file）
    assert _C.LOG_DEST in ["stdout", "file"], err_str.format(_C.LOG_DEST)


def merge_from_file(cfg_file):
    """从指定的配置文件中加载配置内容并合并到当前配置中。"""
...
def reset_cfg():
    """将配置重置为默认配置（_CFG_DEFAULT）"""
    cfg.merge_from_other_cfg(_CFG_DEFAULT)


def load_cfg_from_args(description="Config options."):
    """
    从命令行参数中加载配置文件，设定所有相关选项。
    1. 支持命令行参数 --cfg 指定配置文件路径
    2. 支持覆盖配置文件中的键值对
    3. 设置日志文件路径和随机种子等
    """
    ...
    # 使用 LOG_TIME，如果为 -1 就生成当前时间戳
    if cfg.LOG_TIME != -1:
        # 转成字符串，确保是字符串格式
        time_str = str(cfg.LOG_TIME)
        # 如果是 12 位数字（yymmddHHMMSS），就插入一个下划线
        if len(time_str) == 12 and time_str.isdigit():
            current_time = time_str[:6] + "_" + time_str[6:]
        else:
    ...
    # 构造日志文件名
    suffix = f"_{cfg.LOG_SUFFIX}" if cfg.LOG_SUFFIX != -1 else ""
    # 从配置文件名中提取纯名（不带扩展名）
    cfg_name = os.path.splitext(os.path.basename(args.cfg_file))[0]

    # 生成日志文件名（仅包含时间戳）
    suffix = f"_{cfg.LOG_SUFFIX}" if cfg.LOG_SUFFIX != -1 else ""
    log_dest = f"{current_time}{suffix}.txt"

    # 在 SAVE_DIR 下创建 dataset/cfg_name 目录
    cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{cfg.CORRUPTION.DATASET}", cfg_name)
    g_pathmgr.mkdirs(cfg.SAVE_DIR)

    # 更新 cfg 中日志字段
    cfg.LOG_TIME = current_time
    cfg.LOG_DEST = log_dest
    cfg.freeze()

    # 🔥 关键修改：参数(cfg内容)只写文件，不打印到控制台
    file_handler = logging.FileHandler(os.path.join(cfg.SAVE_DIR, cfg.LOG_DEST))
    ...
    # 🔥 打印cfg只写到文件
    cfg_logger = logging.getLogger("cfg_only")
    cfg_logger.addHandler(file_handler)     # 只写文件
    cfg_logger.propagate = False            # 不传到控制台
    cfg_logger.setLevel(logging.INFO)
    cfg_logger.info(cfg)
...
def complete_data_dir_path(data_root_dir: str, dataset_name: str):
    ...
    mapping = {
        ...
        "eurosat": os.path.join("eurosat", "2750"),
        "fgvc_aircraft": os.path.join("fgvc-aircraft-2013b", "data"),
        ...
        "stanford_cars": os.path.join("stanford_cars"),
        "sun397": os.path.join("sun397"),
        "ucf101": os.path.join("ucf101", "UCF-101-midframes"),
        "ccc": "",
        "PU_1d_8c_2048": "PU_1d_8c_2048", "PU": "PU_13c", "PU_13c": "PU_13c", "PU_13c_1d": "PU_13c_1d",
        "PHM": "PHM", "JNU": "JNU_4c", "JNU_4c": "JNU_4c",
    }
...
def ds_name2pytorch_ds_name(ds_name: str):
    """将本地自定义数据集名称映射为torchvision中的标准类名（用于加载）"""
    lookup_table = {
...
def get_num_classes(dataset_name: str):
    ...
    dataset_name2num_classes = {
        "cifar10": 10, "cifar10_c": 10, "cifar100": 100,  "cifar100_c": 100,
        ...
        "PU_1d_8c_2048": 8, "PU_13c": 13, "PU_13c_1d": 13, "PHM": 14, "JNU_4c": 4,
    }
...
def ckpt_path_to_domain_seq(ckpt_path: str):
    """从 checkpoint 路径中解析出该模型所属源域，从而推断出使用的适应顺序"""
...
def load_config():
    ...
    # 自动记录当前时间作为日志标签
    from datetime import datetime
    cfg.LOG_TIME = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')

    return cfg


def setup_logger(name='source_train', level=logging.INFO):
    logger = logging.getLogger(name)
    logger.setLevel(level)

    if not logger.handlers:
        formatter = logging.Formatter("[%(asctime)s] [%(filename)s:%(lineno)4d]: %(message)s", datefmt='%d/%m/%y %H:%M:%S')
